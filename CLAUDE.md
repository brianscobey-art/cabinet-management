...
- **COAST suite launcher** (2026-08-07): login lands on `#/suite` — five tiles with Carter-green SVG marks: Cabinetron (→ #/jobs), Optimus (→ /ordering-platform), Autobot (→ /autobot), Sterling and Tailgate (coming-soon, disabled). The ⠿ button at the far left of the header reopens it from anywhere. `frontend/src/pages/SuitePage.tsx`.
